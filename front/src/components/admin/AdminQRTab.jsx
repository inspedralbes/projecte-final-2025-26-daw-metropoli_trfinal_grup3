...
const AdminQRTab = () => {
    ...
    const handleGenerateQR = async () => {
        ...
        try {
            ...
            alert(t("admin.qrs.alerts.generate_error", "Error al generar el QR. Comprueba que el backend esté conectado."));
        } finally {
        ...
        <div className="animate-fade-in lg:grid lg:grid-cols-2 lg:gap-8 lg:items-start space-y-8 lg:space-y-0">
            ...
            <div className="order-2 lg:order-1">
                <div className="mt-6 lg:mt-0">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 ml-1 lg:mb-3">
                        {t("admin.qrs.generated_title", "QRs Generados")}
                    </h3>
                    ...
                    {qrList.length > 0 ? (
                                ...
                                >
                                        ...
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {qr.activo ? (
                                                <span className="bg-emerald-100 text-emerald-600 text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                                                    {t("admin.qrs.active", "Activo")}
                                                </span>
                                            ) : (
                                                <span className="bg-slate-100 text-slate-500 text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap">
                                                    {t("admin.qrs.inactive", "Inactivo")}
                                                </span>
                                            )}
                                        </div>

                                        <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                                            {t("admin.qrs.node_id", "Nodo ID")}: {qr.id_nodo_inicio}
                                            {qr.nodo_descripcion && ` - ${qr.nodo_descripcion}`}
                                        </p>

                                        <p className="text-[11px] text-slate-400 mt-1">
                                            {t("admin.qrs.date", "Fecha")}: {new Date(qr.fecha_creacion).toLocaleDateString('es', { day: 'numeric', month: 'short', year: 'numeric' })}
                                        </p>
                                    </div>
                                    ...
                                    <div className="shrink-0 flex gap-2">
                                        <a
                                            ...
                                            title={t("admin.qrs.download_tooltip", "Descargar Imagen QR")}
                                        >
                        ...
                        <div className="bg-slate-50 dark:bg-[#12080a] text-slate-400 dark:text-slate-500 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 p-8 text-center flex flex-col items-center justify-center shadow-inner">
                            ...
                            <p className="font-medium text-sm">{t("admin.qrs.no_qrs", "No se han generado Códigos QR.")}</p>
                            <p className="text-xs mt-1 text-slate-400">{t("admin.qrs.generate_instructions", "Genera uno en el panel de la derecha.")}</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="order-1 lg:order-2">
                <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    {t("admin.qrs.generator_title", "Generador de QR Topográfico")}
                </h3>
                ...
                <div className="bg-white dark:bg-[#12080a] rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 space-y-6">
                    ...
                    <div>
                        <label className="block text-xs font-bold uppercase text-slate-400 mb-2 ml-1">
                            {t("admin.qrs.start_node", "Nodo de Inicio (Punto Físico)")}
                        </label>
                        <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 py-3 border border-slate-100 dark:border-slate-700 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                            ...
                            {nodosList.length > 0 ? (
                                ...
                                >
                                    <option value="" disabled>{t("admin.qrs.select_node_placeholder", "Selecciona un nodo del circuito...")}</option>
                                    {nodosList.map(nodo => (
                                ...
                                <select disabled className="bg-transparent border-none outline-none w-full text-slate-400 text-sm font-medium appearance-none">
                                    <option>{t("admin.qrs.no_nodes_available", "No hay nodos de navegación disponibles")}</option>
                                </select>
                            )}
                        </div>
                        <p className="text-[10px] text-slate-400 mt-1.5 ml-1">
                            {t("admin.qrs.node_warning", "Asegúrate de que el nodo seleccionado coincida con el punto real donde imprimirás este cartel QR para la navegación en AR.")}
                        </p>
                    </div>
                    ...
                    <div>
                        <label className="block text-xs font-bold uppercase text-slate-400 mb-2 ml-1">
                            {t("admin.qrs.zone_name", "Nombre de Zona (Opcional)")}
                        </label>
                        <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800 rounded-xl px-4 py-3 border border-slate-100 dark:border-slate-700 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                            <span className="material-symbols-outlined text-slate-400">
                                title
                            ...
                            <input
                                ...
                                placeholder={t("admin.qrs.zone_placeholder", "e.g. tribuna-sur")}
                            />
                        </div>
                        <p className="text-[10px] text-slate-400 mt-1.5 ml-1">
                            {t("admin.qrs.zone_name_desc_1", "Por defecto se guardará como")} <strong>{t("admin.qrs.zone_name_desc_2", "zona-sin-definir.png")}</strong>. {t("admin.qrs.zone_name_desc_3", "Se usará este texto como nombre de archivo en servidor.")}
                        </p>
                    </div>
                    ...
                    <div className="pt-2">
                        ...
                        >
                            ...
                            <span className="relative z-10">
                                {isGenerating ? t("admin.qrs.generating", "Generando e Insertando...") : t("admin.qrs.generate_btn", "Generar Nuevo Código QR")}
                            </span>
                        </button>
                    </div>
