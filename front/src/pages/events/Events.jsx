...
const Events = () => {
    ...
    // Leemos el usuario del localStorage una sola vez al montar el componente
    useEffect(() => {
        ...
        getEventos().then((res) => {
            ...
            for (let i = 0; i < res.data.length; i++) {
                ...
                nuevoEventosPorDia[claveDelDia].push({
                    ...
                    image: evento.foto
                        ? (import.meta.env.VITE_API_URL || 'http://localhost:3000') + evento.foto
                        : null
                });
            }
    ...
    // URL de la foto del usuario (con fallback a placeholder de iniciales)
    let fotoUsuario = null;
    if (usuario && usuario.foto) {
        fotoUsuario = (import.meta.env.VITE_API_URL || 'http://localhost:3000') + usuario.foto;
    }
    ...
    return (
        <div className="min-h-screen w-full bg-gray-50 dark:bg-slate-950 text-slate-800 dark:text-white font-display select-none transition-colors duration-300 md:pl-16">
            ...
            <div className="w-full pt-6 px-5 pb-2 z-20 flex justify-between items-center transition-colors shrink-0 touch-none md:max-w-5xl md:mx-auto">
                ...
                <Link to="/profile" className="w-10 h-10 rounded-full border-2 border-primary p-0.5 overflow-hidden shadow-sm flex-shrink-0">
                    {fotoUsuario ? (
                        <img src={fotoUsuario} alt="Profile" className="w-full h-full object-cover rounded-full" />
                    ) : (
                ...
                <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-8 lg:items-start">
                    ...
                    <div className="mb-6 lg:mb-0">
                            ...
                            {diasConEventos.map((diaInfo) => (
                                ...
                                >
                                    <div className="lg:flex lg:flex-col lg:items-center lg:w-10">
                                        ...
                                        <span className="text-lg font-black mt-0.5 block">{diaInfo.dia}</span>
                                    </div>
                                    <span className={`hidden lg:block text-sm font-semibold ${activeTab === diaInfo.clave ? 'text-white/80' : 'text-slate-500 dark:text-slate-400'}`}>
                                        {diaInfo.mes}
                                    </span>
                                </button>
                            ))}
                        ...
                        <div className="hidden lg:block mt-4 p-4 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm">
                            <div className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">{t('events.sessionsToday')}</div>
                                    ...
                                    <div className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-md dark:shadow-none transition-all hover:scale-[1.01] active:scale-[0.99]">
                                        ...
                                        <div className="p-4 relative z-10">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <div className="px-2 py-0.5 bg-slate-100 dark:bg-white/5 rounded text-[9px] font-bold text-slate-500 dark:text-gray-400 uppercase tracking-widest border border-slate-200 dark:border-white/5">{t('events.mainEvent')}</div>
                                            </div>
                                        ...
                                        {event.image && (
                                            <div className="h-16 relative mt-1">
                                                <img alt={event.title} className="w-full h-full object-cover opacity-50 dark:opacity-30 grayscale" src={event.image} />
                                    ...
                                    <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex items-center justify-between shadow-sm dark:shadow-none transition-all hover:scale-[1.01] active:scale-[0.99] ${event.status === 'completed' ? 'opacity-60 border-dashed' : ''}`}>
                                        <div>
                                            ...
                                            {event.category && event.status !== 'completed' && (
                                                <p className="text-[10px] text-slate-500 dark:text-gray-500 mt-0.5 font-medium">{event.category}</p>
                                            )}
                                        </div>
