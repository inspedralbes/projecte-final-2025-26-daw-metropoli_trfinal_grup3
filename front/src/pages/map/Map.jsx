...
const Map = () => {
  ...
  const handleLocate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (mapRef.current) mapRef.current.flyTo(newPos, 16, { animate: true, duration: 1.5 });
      },
  ...
  return (
    <div
      className="relative h-[100dvh] w-full bg-gray-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-display overflow-hidden select-none transition-colors duration-300 overscroll-none md:pl-16"
    ...
    >
      ...
      <div className="relative z-30 flex flex-col h-full pointer-events-none">
        ...
        <div className="w-full pt-6 px-5 pointer-events-auto touch-none">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="md:hidden flex items-center gap-2">
              <Link to="/home">
                <img src="/logo/logo1.png" alt="Circuit Logo" className="h-12 w-auto object-contain block dark:hidden" />
                <img src="/logo/logo.png" alt="Circuit Logo" className="h-12 w-auto object-contain hidden dark:block" />
              </Link>
            </div>
            <h1 className="hidden md:block text-2xl font-black italic uppercase tracking-tighter text-white drop-shadow-lg">
              Circuit <span className="text-primary">Map</span>
            </h1>
            <Link to="/profile" className="w-10 h-10 rounded-full border-2 border-primary p-0.5 overflow-hidden shadow-sm bg-white dark:bg-slate-900">
        ...
        {/* Legend Toggle Tab (Left Side) — on desktop, shifts with sidebar offset */}
        <button
        ...
        <div className={`absolute left-0 top-0 bottom-0 w-64 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-r border-slate-200 dark:border-slate-800 shadow-2xl transition-transform duration-300 z-40 pointer-events-auto flex flex-col justify-center px-6 ${isLegendOpen ? "translate-x-0" : "-translate-x-full"}`}>
          <div className="mb-6">
            <span className="text-xs uppercase tracking-widest font-bold text-slate-400 block mb-6 border-b border-slate-100 dark:border-slate-800 pb-2">Map Legend</span>
            <div className="space-y-6">
              {legendItems.map(({ color, label }) => (
                <div key={label} className="flex items-center gap-4 group">
                  <div className={`w-8 h-8 rounded-full ${color}/10 flex items-center justify-center group-hover:${color}/20 transition-colors`}>
        ...
        {/* Right Side Controls */}
        <div className="absolute right-5 bottom-32 md:bottom-10 flex flex-col gap-4 pointer-events-auto z-40">
          <button
